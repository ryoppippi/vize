...
fn a_manifest_naming_an_unknown_group_or_kind_is_refused_exactly() {
    let document = PluginDocument::build(TODOS, "Todos.vue").expect("splits");
    let mut manager = FactManager::new(&REGISTRY);
    let bindings = strings(&["future-facts"]);
    let error =
        build_batch(&document, &spec(None, &bindings), &mut manager).expect_err("unknown fact");
    assert_eq!(
        error.to_string(),
        "team: fact group `future-facts` is not available to JS plugins (available: templateScopes, bindings, undefined-refs, component-usages, reactivity, provide-inject, race-conditions, unused-bindings, component-signature, prop-types, emit-types, slot-types, reactivity-classes, component-references)"
    );
