...
**Deliverable:** `docs/davinci/contracts-compat-policy.md` (written before GA) and Marquette-style canonical serialization with additive/breaking classification for contract payloads, extending `crates/vize_marquette/src/compatibility*`; the conformance suite is versioned alongside the contracts.
...
**Deliverable:** `docs/davinci/completion-metrics.md`: charter #35's pinned numbers against achieved — compile throughput, peak memory, keystroke p95, fact adoption, source-map coverage, matrices without orphans — with each miss explained or remediated.
