...
import { copyGridDataToClipboard, equalCellAddress, getCellAddress, getCellElement, pasteToGridFromClipboard, removeDataFromGrid } from '@/components/grid/grid-utils.js'
...
export default /*@__PURE__*/_defineComponent({
  __name: 'MkGrid',
  props: {
    settings: { type: null as unknown as PropType<GridSetting>, required: true },
    data: { type: Array as PropType<DataSource[]>, required: true }
  },
  emits: ["event"],
...
return (_ctx: any,_cache: any) => {
  return (_openBlock(), _createElementBlock("div", {
      ref: rootEl,
      class: _normalizeClass(["mk_grid_border", [_ctx.$style.grid, {
  		[_ctx.$style.noOverflowHandling]: _unref(rootSetting).noOverflowStyle,
  		'mk_grid_root_rounded': _unref(rootSetting).rounded,
  		'mk_grid_root_border': _unref(rootSetting).outerBorder,
  	}]]),
      onMousedown: _withModifiers(onMouseDown, ["prevent"]),
      onKeydown: onKeyDown,
      onContextmenu: _withModifiers(onContextMenu, ["prevent","stop"])
    }, [ _createElementVNode("div", { class: "mk_grid_thead" }, [ _createVNode(MkHeaderRow, {
          columns: columns.value,
          gridSetting: _unref(rowSetting),
          bus: _unref(bus),
          "onOperation:beginWidthChange": onHeaderCellWidthBeginChange,
          "onOperation:endWidthChange": onHeaderCellWidthEndChange,
          "onOperation:widthLargest": onHeaderCellWidthLargest,
          "onChange:width": onHeaderCellChangeWidth,
          "onChange:contentSize": onHeaderCellChangeContentSize
        }) ]), _createElementVNode("div", { class: "mk_grid_tbody" }, [ (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(rows.value, (row) => {
          return _withDirectives((_openBlock(), _createBlock(MkDataRow, {
            key: row.index,
            row: row,
            cells: cells.value[row.index].cells,
            setting: _unref(rowSetting),
            bus: _unref(bus),
            using: row.using,
            "onOperation:beginEdit": onCellEditBegin,
            "onOperation:endEdit": onCellEditEnd,
            "onChange:value": onChangeCellValue,
            "onChange:contentSize": onChangeCellContentSize
          }, null, 8 /* PROPS */, ["row", "cells", "setting", "bus", "using"])), [
            [_vShow, row.using]
          ])
