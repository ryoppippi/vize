...
<template>
  <div class="panel input-panel">
    <div class="panel-header">
      <h2>
        {{ inputMode === "sfc" ? "SFC (.vue)" : "Template" }}
      </h2>
      <div class="panel-actions">
        <button class="btn-ghost" @click="handlePresetChange(selectedPreset)">
          Reset
        </button>
        <button class="btn-ghost" @click="copyToClipboard(source)">
          Copy
        </button>
      </div>
    </div>
    ...
    isTypeScript && codeViewMode === "js" ? formattedJsCode : formattedCode || output.code,
        ...
        <div v-else-if="activeTab === "ast"" class="ast-output">
          <div class="ast-header">
            ...
            <div class="ast-options">
              ...
              <button class="btn-ghost btn-small" @click="copyToClipboard(astJson)">
                Copy
              </button>
            </div>
          </div>
        ...
        <div v-else-if="activeTab === "sfc" && sfcResult" class="sfc-output">
          ...
          <div v-if="sfcResult.descriptor.template" class="sfc-block">
            ...
            <CodeHighlight language="html" :code="sfcResult.descriptor.template.content" :theme />
          </div>
          ...
          <div v-if="sfcResult.descriptor.script" class="sfc-block">
            ...
            <CodeHighlight language="typescript" :code="sfcResult.descriptor.script.content" :theme />
          </div>
          <div v-if="sfcResult.descriptor.styles?.length > 0" class="sfc-block">
            <h5>
              Styles ({{ sfcResult.descriptor.styles?.length }})
            </h5>
            <div v-for="(style, i) in sfcResult.descriptor.styles" :key="i" class="style-block">
              <span class="style-meta">
                <span v-if="style.scoped" class="badge">
                  scoped
                </span>
              ...
              <CodeHighlight language="css" :code="style.content" :theme />
            </div>
          </div>
        </div>
        <!-- CSS Tab -->
        <div v-else-if="activeTab === "css"" class="css-output">
          ...
          <p v-else class="no-css">
            No styles in this SFC
          </p>
        </div>
        <!-- Bindings Tab -->
        <div
