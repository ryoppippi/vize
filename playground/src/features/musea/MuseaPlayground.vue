  ...
  <div class="musea-playground">
    <div class="panel input-panel">
      <div class="panel-header">
        ...
        <div class="panel-actions">
          <a class="btn-examples" href="/musea-examples/__musea__" rel="noopener" target="_blank">
            <svg
              fill="none"
              height="12"
              stroke="currentColor"
          ...
          <button class="btn-ghost" @click="source = ART_PRESET">
            Reset
          </button>
        </div>
      </div>
    ...
    <div class="panel output-panel">
      <div class="panel-header">
        <div class="header-title">
          <svg class="icon" viewBox="0 0 24 24">
            <path fill="currentColor" :d="mdiDiamond" />
          </svg>
          <h2>
            Art Analysis
          </h2>
          ...
          <div v-if="activeTab === "parsed"" class="parsed-output">
            <div class="output-header-bar">
              ...
              <div class="file-badges">
                <span v-if="parsedArt.hasScriptSetup" class="file-badge">
                  setup
                </span>
                <span v-if="parsedArt.hasScript" class="file-badge">
                  script
                </span>
                <span v-if="parsedArt.styleCount > 0" class="file-badge">
                  {{ parsedArt.styleCount }} style
                </span>
              </div>
            </div>
            <div class="metadata-section">
              <div class="metadata-grid">
                ...
                <div v-if="parsedArt.metadata.description" class="metadata-item span-full">
                  ...
                  <span class="meta-value">
                    {{ parsedArt.metadata.description }}
                  </span>
                </div>
                ...
                <div v-if="parsedArt.metadata.category" class="metadata-item">
                  ...
                  <span class="meta-value category-value">
                    {{ parsedArt.metadata.category }}
                  </span>
                </div>
            ...
            <template v-if="designTokens.length > 0">
              ...
              <div v-if="colorTokens.length > 0" class="token-section">
                ...
                <div class="color-grid">
                  ...
                  >
                    <div class="color-swatch" :style="{ background: token.value }">
                    </div>
              ...
              <div v-if="sizeTokens.length > 0" class="token-section">
                ...
                <div class="token-list">
                  ...
                  >
                    ...
                    <div class="size-preview" :style="{ width: token.value }">
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-for="variant in parsedArt.variants" :key="variant.name" class="variant-item">
                <div class="variant-header">
                  <div class="variant-name">
                    {{ variant.name }}
                    <span v-if="variant.isDefault" class="default-badge">
                      default
                    </span>
                    <span v-if="variant.skipVrt" class="skip-badge">
                      skip vrt
                    </span>
                  </div>
                  <button class="btn-copy" @click="copyToClipboard(variant.template)">
                    Copy
                  </button>
                </div>
                <div class="variant-template">
                  <CodeHighlight language="html" :code="variant.template" :theme />
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === "csf" && csfOutput" class="csf-output">
            <div class="output-header-bar">
              ...
              <div class="csf-actions">
                <code class="filename-badge">
                  {{ csfOutput.filename }}
                </code>
                <button class="btn-copy" @click="copyToClipboard(csfOutput.code)">
                  Copy
                </button>
              </div>
            </div>
            <div class="code-container">
              <CodeHighlight language="typescript" show-line-numbers :code="csfOutput.code" :theme />
            </div>
          </div>
        </template>
        <div v-else class="loading-state">
          <span>
            Enter an Art file to see the output
          </span>
        </div>
      </div>
    </div>
