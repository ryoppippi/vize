...
const {
  palette, loading, error, values,
  allControls, mergedValues, customPropNames,
  load, setValue, setAllValues, addProp, removeProp, resetValues,
} = usePalette()
...
const usageCode = computed(() => {
  ...
  const propsEntries = Object.entries(mergedValues.value).filter(([, v]) => v !== undefined && v !== '')
          ...
          <template v-if="controlsMode === 'controls'">
            <div class="props-grid">
              <template v-for="group in (palette.groups || [])" :key="group">
                ...
                <template v-for="control in allControls.filter(c => c.group === group)" :key="control.name">
                  <div class="props-control-row" :class="{ 'props-control-custom': customPropNames.has(control.name) }">
                    ...
                    <span v-if="customPropNames.has(control.name)" class="props-custom-badge">custom</span>
                    <button
                      type="button"
            ...
            <div class="props-add-section">
              ...
              <div v-else class="props-add-form">
                ...
                <div class="props-add-actions">
                  ...
                  <button type="button" class="props-add-cancel" @click="showAddForm = false">Cancel</button>
                </div>
              </div>
            </div>
          </template>
